...
namespace MathInterpolations
{
    public static class Interpolations
    {
        public static double LinearInterpolation(double[] xarray, double[] yarray, double x)
        {
            double y = 0;
        ...
        public static double[] LinearIntepolation(double[] xarray, double[] yarray, double[] x)
        {
            double[] y = new double[x.Length];
            for (int i = 0; i < x.Length; i++)
                y[i] = LinearInterpolation(xarray, yarray, x[i]);
            return y;
        }

        public static double LagrangianInterpolation(double[] xarray, double[] yarray, double x)
        {
            double y = 0.0;
        ...
        public static double[] LagrangianInterpolation(double[] xarray, double[] yarray, double[] x)
        {
            ...
            for (int i = 0; i < x.Length; i++)
                y[i] = LagrangianInterpolation(xarray, yarray, x[i]);
            return y;
        }

        public static double BarycentricInterpolation(double[] xarray, double[] yarray, double x)
        {
            double product;
            double dx;
            double c1 = 0;
        ...
        public static double[] BarycentricInterpolation(double[] xarray, double[] yarray, double[] x)
        {
            double[] y = new double[x.Length];
            for (int i = 0; i < x.Length; i++)
                y[i] = BarycentricInterpolation(xarray, yarray, x[i]);
            return y;
        }

        public static double NewtonDividedDifferenceInterpolation(double[] xarray, double[] yarray, double x)
        {
            double y;
            int n = xarray.Length;
        ...
        public static double[] NewtonDividedDifferenceInterpolation(double[] xarray, double[] yarray, double[] x)
        {
            double[] y = new double[x.Length];
            for (int i = 0; i < x.Length; i++)
                y[i] = NewtonDividedDifferenceInterpolation(xarray, yarray, x[i]);
            return y;
        }
        ...
        private static double[] SecondDerivatives(double[] xarray, double[] yarray)
        {
            ...
            for (int i = 1; i < n - 1; i++)
            {
                ...
                c1[i - 1] = 1 - c3[i - 1];
                derivative[i - 1] = 6 * (derivative[i + 1] - derivative[i]) /
                                        (dx[i] + dx[i + 1]);
            }
